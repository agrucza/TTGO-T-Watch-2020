...
AppStandby::AppStandby():App("Standby", false)
{
    ...
    // time label
    lv_style_init(&_styleTimeLabel);
    lv_style_set_text_font(&_styleTimeLabel, LV_STATE_DEFAULT, &lv_font_montserrat_48);
    _timeLabel = lv_label_create(_container, NULL);
    lv_obj_add_style(_timeLabel, LV_LABEL_PART_MAIN, &_styleTimeLabel);
    ...
    lv_style_copy(&_styleIconContainer, &GUI::styleBorderless);
    lv_style_set_bg_color(&_styleIconContainer, LV_CONT_PART_MAIN, LV_COLOR_GRAY);
    lv_style_set_pad_inner(&_styleIconContainer, LV_CONT_PART_MAIN, 2);
    lv_style_set_pad_top(&_styleIconContainer, LV_CONT_PART_MAIN, 2);
    lv_style_set_pad_left(&_styleIconContainer, LV_CONT_PART_MAIN, 4);
    lv_style_set_pad_bottom(&_styleIconContainer, LV_CONT_PART_MAIN, 2);
    lv_style_set_pad_right(&_styleIconContainer, LV_CONT_PART_MAIN, 4);

    _iconContainer = lv_cont_create(_container, NULL);
    
    lv_obj_add_style(_iconContainer, LV_CONT_PART_MAIN, &_styleIconContainer);
    ...
    lv_obj_add_style(_touchLabel, LV_LABEL_PART_MAIN, &_styleTimeLabel);
...
void AppStandby::eventCallback(lv_obj_t* obj, lv_obj_t* ext, lv_event_t event, AppCallback* callback)
{
    if(!_touched && event == LV_EVENT_PRESSED){
        _touched = true;
        _gui->getTTGO()->motor->onec(50);
        lv_obj_set_hidden(_touchLabel, false);
    }

    if(event == LV_EVENT_LONG_PRESSED)
    {
        _gui->getTTGO()->motor->onec(100);
        lv_obj_set_hidden(_touchLabel,true);
